...
public interface WorkCommentRepository extends JpaRepository<WorkComment, Long> {
    ...
    List<WorkComment> findByCommentTypeOrderByCreatedAtDesc(@Param("commentType") WorkComment.CommentType commentType);
    ...
    @Query("SELECT wc FROM WorkComment wc WHERE wc.work.vehicle.ownerCui = :clientCui ORDER BY wc.createdAt DESC")
